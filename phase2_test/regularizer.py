...
class MaxNorm_via_PGD():
    ...
    def setPerLayerThresh(self, model):
        ...
        for curLayer in [model.module.prompt_learner.head.weight, model.module.prompt_learner.head.bias]: #here we only apply MaxNorm over the last two layers
    ...
    def PGD(self, model):
        ...
        for i, curLayer in enumerate([model.module.prompt_learner.head.weight, model.module.prompt_learner.head.bias]): #here we only apply MaxNorm over the last two layers
            curparam = curLayer.data
...
class Normalizer(): 
    ...
    def apply_on(self, model): #this method applies tau-normalization on the classifier layer
        ...
        for curLayer in [model.module.prompt_learner.head.weight]: #change to last layer: Done
            curparam = curLayer.data
